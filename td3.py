...
# TD3 특화 하이퍼파라미터
policy_delay   = 2         # Actor를 Critic 대비 몇 step마다 업데이트할지 
policy_noise   = 0.2       # Target action에 추가할 noise 표준편차
noise_clip     = 0.5       # Target action noise 클리핑 범위
expl_noise     = 0.1       # 학습 중 탐색용 Gaussian noise 표준편차

n_episodes     = 10000
print_interval = 20
video_interval = 300
env_name       = "Pendulum-v1"
action_bound   = 2.0       # Pendulum action 범위: [-2, 2]
...
def train(mu, mu_target, q1, q1_target, q2, q2_target,
          memory, q1_optimizer, q2_optimizer, mu_optimizer, step_count):
    ...
    with torch.no_grad():
        ...
        target_q1 = q1_target(s_prime, a_target)
        target_q2 = q2_target(s_prime, a_target)
...
def main():
    ...
    log_dir = os.path.join(os.path.expanduser('~'), 'tb_logs', f'TD3_{time.time():.0f}')
    ...
    for n_epi in range(n_episodes):
        ...
        while count < 200 and not done:
            a = mu(torch.from_numpy(s).float()).item()
            a = a + np.random.normal(0, expl_noise * action_bound)
    ...
    plt.savefig('td3_result.png', dpi=150)
    plt.close()

    print("\nsaved: td3_result.png")
